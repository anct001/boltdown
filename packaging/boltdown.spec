...
#
#     pyinstaller packaging/boltdown.spec --noconfirm
#
# Why three:
#   Boltdown.exe       windowed GUI - the one users launch
#   boltdown-cli.exe   console CLI, and the only way to register the browser
#                      host on a machine with no Python. The name has to differ
#                      from Boltdown.exe by more than case: Windows file names
#                      are case-insensitive, so "boltdown.exe" would silently
#                      overwrite the GUI in the output directory
#   boltdown-host.exe  console native-messaging host; Chrome speaks stdio, so
...
def trim(entries):
    ...
    if len(kept) != len(entries):
        print(f"[boltdown.spec] dropped {len(entries) - len(kept)} unused Qt files")
    return kept
...
# depend on Boltdown.exe being started first - not true for the host, which
